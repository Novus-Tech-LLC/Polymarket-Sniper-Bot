...
export function normalizeOrder(input: OrderInput): Order {
  return {
    ...
    signer: input.signer,
    taker: input.taker,
    tokenId: typeof input.tokenId === "bigint" ? input.tokenId : BigInt(input.tokenId),
    makerAmount: typeof input.makerAmount === "bigint" ? input.makerAmount : BigInt(input.makerAmount),
    takerAmount: typeof input.takerAmount === "bigint" ? input.takerAmount : BigInt(input.takerAmount),
    expiration: typeof input.expiration === "bigint" ? input.expiration : BigInt(input.expiration ?? 0),
    nonce: typeof input.nonce === "bigint" ? input.nonce : BigInt(input.nonce ?? 0),
    feeRateBps: typeof input.feeRateBps === "bigint" ? input.feeRateBps : BigInt(input.feeRateBps ?? 0),
