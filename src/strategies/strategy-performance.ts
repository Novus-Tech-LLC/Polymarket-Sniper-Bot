...
export class StrategyPerformanceTracker {
   ...
   */
  registerStrategy(name: string, baseAllocationPct: number, maxPositionUsd: number): void {
  ...
  recordTrade(
    ...
    metrics.winRate = metrics.tradesExecuted > 0 
      ? metrics.tradesSuccessful / metrics.tradesExecuted 
      : 0;
    metrics.avgProfitPerTrade = metrics.tradesSuccessful > 0
      ? metrics.totalProfitUsd / metrics.tradesSuccessful
      : 0;
    metrics.avgLossPerTrade = metrics.tradesFailed > 0
  ...
  getRecommendedPositionSize(strategyName: string, baseMaxUsd: number): number {
    ...
    const allocationRatio = allocation.currentAllocationPct / allocation.baseAllocationPct;
  ...
  rebalance(): void {
    ...
    const rollingMetrics: Map<string, { roi: number; trades: number }> = new Map();
