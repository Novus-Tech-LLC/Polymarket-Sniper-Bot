...
export const resolvePolymarketContracts = (): PolymarketContracts => {
  ...
  const ctfExchangeAddress = readEnv('POLY_CTF_EXCHANGE_ADDRESS') || POLYMARKET_CTF_EXCHANGE_ADDRESS;
  const negRiskExchangeAddress = readEnv('POLY_NEG_RISK_CTF_EXCHANGE_ADDRESS') || POLYMARKET_NEG_RISK_CTF_EXCHANGE_ADDRESS;
  const negRiskAdapterAddress = readEnv('POLY_NEG_RISK_ADAPTER_ADDRESS') || POLYMARKET_NEG_RISK_ADAPTER_ADDRESS;
  return {
    usdcAddress,
    ctfAddress,
    ctfExchangeAddress,
