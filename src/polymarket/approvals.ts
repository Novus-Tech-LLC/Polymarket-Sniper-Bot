...
export const readApprovalsConfig = (): ApprovalsConfig => {
  ...
  const minUsdcAllowanceRaw =
    readEnv('APPROVAL_MIN_USDC') ?? readEnv('APPROVALS_MIN_USDC_ALLOWANCE') ?? '1000';
  return {
    mode,
    minUsdcAllowanceRaw,
...
export const fetchApprovalSnapshot = async (params: {
  ...
  const usdcContract = new Contract(contracts.usdcAddress, ERC20_ABI, params.wallet.provider ?? params.wallet);
...
export const ensureApprovals = async (params: {
  ...
  const { contracts, usdcSpenders, erc1155Operators } = resolveApprovalTargets();
  if (!contracts.ctfErc1155Address || usdcSpenders.length === 0 || erc1155Operators.length === 0) {
    params.logger.warn('[Preflight][Approvals] Missing spender/operator addresses; approvals cannot be performed.');
    const snapshot = await fetchApprovalSnapshot({ wallet: params.wallet, owner: params.owner, logger: params.logger });
  ...
  const approvalAmount = params.config.approveMaxUint ? constants.MaxUint256 : minAllowance;
  const approvalsToSend: Array<{ label: string; to: string; data: string; type: 'erc20' | 'erc1155'; spender: string }> = [];
  ...
  snapshot.allowances.forEach(({ spender, allowance }) => {
    ...
    if (decision.needsErc20) {
      approvalsToSend.push({
        ...
        data: erc20Interface.encodeFunctionData('approve', [spender, approvalAmount]),
        type: 'erc20',
        spender,
      });
    }
  ...
  if (params.config.mode === 'false') {
    approvalsToSend.forEach((approval) => {
      params.logger.warn(`[Preflight][Approvals] Required ${approval.label} token=${approval.to}`);
    });
  ...
  if (params.config.mode === 'dryrun') {
    approvalsToSend.forEach((approval) => {
      params.logger.info(`[Preflight][Approvals][DryRun] ${approval.label} to=${approval.to} data=${approval.data}`);
    });
  ...
  const approvalsOk = refreshed.allowances.every(({ allowance }) => allowance.gte(minAllowance))
