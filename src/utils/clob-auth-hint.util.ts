export const formatClobAuthFailureHint = (deriveEnabled: boolean): string => {
  ...
  return [
    ...
    "2) If using manual API keys (POLYMARKET_API_*), verify they are CLOB API keys from CLOB_DERIVE_CREDS=true (there is no web UI to manually generate CLOB API keys)",
