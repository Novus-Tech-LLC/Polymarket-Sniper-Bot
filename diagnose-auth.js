...
async function checkEnvironmentVariables() {
  ...
  const optional = ['POLYMARKET_API_KEY', 'POLYMARKET_API_SECRET', 'POLYMARKET_API_PASSPHRASE'];
  ...
  if (!hasManualCreds) {
    log(`ℹ️  Manual CLOB credentials not found - will try auto-derive`, 'yellow');
  }
...
async function checkWalletConnection() {
  ...
  try {
    ...
    if (balanceEth > 0.01) {
      ...
      log(`⚠️  POL Balance: ${balanceEth.toFixed(4)} POL (LOW - need gas for transactions)`, 'yellow');
    }
...
async function testAuthenticationFlow(wallet) {
  ...
  try {
    ...
    log('\nStep 3: Verifying credentials with signature type auto-detection...', 'blue');
    ...
    log(`   This suggests credentials are invalid or there's an API issue`, 'yellow');
    return false;
...
async function main() {
  console.log('\n');
  log('╔═══════════════════════════════════════════════════════════════╗', 'blue');
  log('║  Polymarket Authentication Diagnostic Tool                     ║', 'blue');
  log('╚═══════════════════════════════════════════════════════════════╝', 'blue');
  ...
  if (!walletResult) {
    log('\n❌ Wallet connection failed - please check RPC_URL and PRIVATE_KEY', 'red');
    process.exit(1);
  }
