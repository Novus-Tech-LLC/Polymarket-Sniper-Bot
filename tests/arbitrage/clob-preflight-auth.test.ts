...
test('auth_ok is set to true when preflight succeeds with HTTP 200', () => {
  ...
  const preflightCode = fs.readFileSync(
    path.join(__dirname, '../../src/polymarket/preflight.ts'),
    'utf-8'
  );
...
test('auth_ok is set to false when preflight returns 401', () => {
  ...
  assert.ok(
    preflightCode.includes('if (preflight && !preflight.ok && (preflight.status === 401 || preflight.status === 403))') &&
...
test('readyToTrade depends on authOk being true', () => {
  ...
  assert.ok(
    preflightCode.includes('const readyToTrade = !detectOnly && approvalsOk && authOk'),
...
test('auth_ok is not set based on credential existence alone', () => {
  ...
  assert.ok(
    !preflightCode.includes('const authOk = params.clobCredsComplete || params.clobDeriveEnabled'),
  ...
  const firstAuthOkSet = preflightCode.indexOf('authOk = true', authOkDeclaration);
  const readyToTradeCheck = preflightCode.indexOf('const readyToTrade = !detectOnly && approvalsOk && authOk');
  
  assert.ok(authOkDeclaration > 0, 'authOk should be declared');
  assert.ok(firstAuthOkSet > authOkDeclaration, 'authOk should be set after declaration');
  assert.ok(readyToTradeCheck > firstAuthOkSet, 'readyToTrade check should come after authOk is set');
});
...
test('preflight tool exits with non-zero code when not ready', () => {
  const toolCode = fs.readFileSync(
    path.join(__dirname, '../../src/tools/preflight.ts'),
    'utf-8'
  );
