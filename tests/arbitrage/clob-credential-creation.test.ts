...
const PRIVATE_KEY = '0x59c6995e998f97a5a0044976f9d1f4aa2e9d8f99a6a1c4b7c1b9f8e178b0ff5d';
...
test('POST /auth/api-key 400 response: no cache file written, credentials not saved', async () => {
  ...
  const mockClient = {
    create_or_derive_api_creds: async () => {
      const error = new Error('Could not create api key') as Error & { response?: { status: number; data: unknown } };
      error.response = { status: 400, data: { error: 'Could not create api key' } };
      throw error;
    },
  };

  // Import the function we need to test
  const { createPolymarketClient } = await import('../../src/infrastructure/clob-client.factory');
  ...
  assert.equal(fs.existsSync(TEST_CREDS_PATH), false, 'Cache file should not exist after 400 response');
});
...
test('POST /auth/api-key returns incomplete creds: no cache file written', async () => {
  ...
  const factoryCode = fs.readFileSync(
    path.join(__dirname, '../../src/infrastructure/clob-client.factory.ts'),
    'utf-8'
  );
  
  // Check that validation logic exists
  assert.ok(
    factoryCode.includes('!derived || !derived.key || !derived.secret || !derived.passphrase'),
...
test('Successful POST /auth/api-key with valid response: cache written with credentials', () => {
  ...
  assert.ok(
    factoryCode.includes('saveCachedCreds({ creds: derived, signerAddress, logger })'),
...
test('Cached credentials verification on startup: invalid creds cleared and retry', () => {
  ...
  assert.ok(
    factoryCode.includes('Cached credentials invalid; clearing cache and retrying derive'),
